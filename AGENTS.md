...
- §22.1 のレビュー観点（A 外部入力検証 / B 文字列後処理 / C ドメイン不変条件 / D preset越境 / E リソース / F テスト）が **3分類以上混在**する
...
PR本文には `## PR Budget` ブロックを必ず含めます（`.github/pull_request_template.md` に空欄あり）。次を満たさないPRはレビュー前に差し戻して構いません。

- `## PR Budget` ブロックが存在しない
- `OBSERVATIONS` が推定値のまま確認されていない
...
2. `.github/workflows/pr-budget.yml` がPR作成・更新時に計測し、超過かつ申告なしでジョブを失敗させる
