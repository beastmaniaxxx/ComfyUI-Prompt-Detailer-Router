...
#### 17.3.1 根拠確認ゲート（修正着手前に必ず実施）

**指摘を受けたら、コードを触る前に、その指摘の根拠が承認済み requirements / design に実在するかを確認してください。**

- 指摘が引用する節番号を実際に開き、**引用された条文が指摘の主張を実際に述べているか**を確認する。節番号が付いているだけでは根拠とみなさない
- 根拠が実在する場合のみ、通常どおり修正へ進む（§17.3.2）
- 根拠が実在しない場合は、**実装で答えず** §21 の分岐へ回す

未仕様の挙動を実装側の判断で埋めると、「正しい答え」が定義されないまま実装だけが動きます。その状態ではレビューのたびに新しい解釈が生まれ、修正が次の指摘を生む無限ループになります。ラウンド上限（§17.1）はこのループを10ラウンドで打ち切るだけで、原因は塞ぎません。

**実測（PR#4 / `domain/forbidden_terms.py`）**: Req 9.3 と design の Postconditions が定めていたのは「除去 + 空白正規化 + `removed_count`」のみでした。区切り修復・空括弧修復・省略記号保持・除去位置限定はいずれも仕様に存在せず、レビュー指摘だけで発生した未仕様挙動です。指摘は一貫して「Requirement 9.3」を根拠として引用していましたが、Req 9.3 は区切り修復を要求していません。結果、63 行の実装が 253 行へ肥大し、このファイルだけで 8 ラウンドを消費しました。仕様（Req 9.6 / 9.7 の決定表）を確定させた後は、続く 3 件の指摘がいずれも「決定表への違反（実装バグ）」か「決定表の文言不足（仕様の明確化）」に機械的に分類でき、1 ラウンドで収束しています。

根拠が仕様ではなく本ファイル（AGENTS.md）の節である場合も同様に確認してください。本ファイルは §2 の優先順位で requirements / design より下位です。**過去の指摘履歴から本ファイルへ書き足された記述を、後続の指摘の根拠として再利用すると循環参照になります**（PR#4 では §22.3 がこれに該当しました）。
...
## Remediation Report
- FINDING: <指摘ID / 要約>
- SPEC_BASIS: <requirements / design の節番号と、その条文が主張を述べていることの確認結果。
               根拠が実在しない場合は NONE とし、§21 のどの分岐で処理したかを書く>
- ROOT_CAUSE: <原因>
- FIXED_AT: <直接指摘された箇所>
...
- `SPEC_BASIS` が空、または節番号を書いただけで条文の確認結果が無い再提出も、同様にREJECTしてよい
...
- 指摘の根拠が requirements / design に実在するかを確認せず修正へ着手する（§17.3.1）
- 仕様に定義の無い挙動を、仕様を確定させないまま実装側の判断で埋める（§21.2 / §21.3）
- 過去の指摘履歴から本ファイルへ書き足した記述を、後続の指摘の根拠として再利用する（循環参照・§17.3.1）
- 動作・契約・仕様適合に影響しない軽微な事項を `REJECTED` の理由にする（§17.5）
- 再現条件を示せない推測ベースの指摘でレビューラウンドを消費する（§17.5）
- PR規模を計測せずにPRを作成する（§19.4）
- テスト行数や仕様文書の行数を §19.2 の分割閾値に数える（§19.2 の区分定義に反する）
...
仕様文書と実装を同一PRに入れないでください。レビュー中に根拠側を書き換えられる状態になり、§17.3.1 の根拠確認ゲートが機能しません。

| フェーズ | PR単位 | 根拠 |
|---|---|---|
| init | 単独PRにしない。requirements PRへ同梱する | `spec.json` と requirements の骨のみでレビュー対象が存在しない |
| requirements | 1PR | mainへマージすることで根拠条文が固定される。`spec.json.approvals` はローカル状態にすぎず、mainに存在する条文とは強度が違う |
| design + tasks | 1PR。design が800行を超える場合のみ分離する | tasks は design からの導出物であり、単独では boundary と完了条件しかレビュー対象にならない |
| impl | tasks.md の `_PR:_` スライス単位（§19.5）。大項目を依存が一方向に閉じる束にまとめて3〜4PR | §19.2 の閾値に従う |

ブランチ名はフェーズ接尾辞で統一します。

```text
spec/<feature>_requirements
spec/<feature>_design
feat/<feature>_<スライス名>
```

### 19.2 実装PRの分割閾値

閾値は**レビュー負荷を持つ行（REVIEW区分）だけ**を対象に判定します。区分は次のとおりで、`tools/pr_budget.py` が同じ定義で機械的に集計します。

| 区分 | 対象パス | 閾値 |
|---|---|---|
| REVIEW | `prompt_detailer_router/**`（`resources/` を含む）、`web/**`、その他の実装ファイル | **追加1,500行** / **30ファイル** |
| TEST | `tests/**` | 上限なし。3,000行超で注意喚起のみ |
| SPEC | `.kiro/specs/**`（`tasks.md` を除く）、`docs/**` | 実装と同一PRに入れない（§19.1） |
| META | `.kiro/steering/**`、`.kiro/specs/**/tasks.md`、`.agents/**`、`.github/**`、`tools/**`、`AGENTS.md` 等 | 対象外 |

次のいずれか1つでも超える場合は分割してください。

- REVIEW区分の**追加1,500行**を超える
- REVIEW区分の変更が**30ファイル**を超える
- §22.1 のレビュー観点が **3分類以上混在**する

**テスト行数を閾値に数えない理由**: §14 はユニットテストの追加を必須とし、§19.3 はそれを実装と同一PRに含めることを要求しています。そのテスト行を分割閾値に数えると、規約どうしが矛盾して「守れない規約」になります。削除行も数えません（大量削除はレビュー負荷に比例しないため）。

**観点分類の数え方**: 数えるのは **A 外部入力検証 / B 文字列後処理 / C ドメイン不変条件 / D preset越境 / E リソース の5分類**です。**F テストは数えません。** §14 が全実装PRにテスト追加を要求するため、F を数えると実装が2分類あるだけでほぼ常に条件が成立し、「テスト行を閾値に数えない」という本節の判断と同じ矛盾を第3条件で再発させるためです。判定対象は REVIEW 区分のファイルに限ります。

分類は最終的に作成者が判断します。`tools/pr_budget.py` はパスから推定値を出しますが、これは申告の下書きであり確定値ではありません。CIは**申告された分類**で判定します（§19.4）。

**実測（PR#7）**: 22ファイル / +2,938行で閾値超過と指摘されましたが、内訳は REVIEW 1,094行 / TEST 1,821行でした。超過の主因は §14 が必須化したテストコードであり、実装規模は閾値内です。この区分を導入する前の「差分1,500行」は、この種の誤検知を構造的に起こしていました。

**実測（PR#2）**: core spec docs 5ファイル（1,135行）と実装6章を同一PRに入れた結果、96ファイル / +5,485行 / 観点6分類となり、3閾値すべてを超過しました。指摘は #2 と #4 で計46件、`forbidden_terms.py` 単体で8ラウンドを消費しています。docs PR（#1）も11ファイル / +2,866行で48件でした。

`prompt-detailer-core` を閾値に従って切り直す場合の分割例です。

```text
PR-1: 1章 + 2章   骨格 + schema / preset / policy 定義   … 分類D
PR-2: 3章         domain                                … 分類B + C
PR-3: 4章         infrastructure                        … 分類A（18件と最大のため単独）
PR-4: 5章 + 6章   application + 横断validation           … 分類E / F
```

大項目1つが単独では小さすぎる場合（1章のパッケージ骨格など）は、依存する直後の章と束ねてください。「1章 = 1PR」の機械適用はしません。

### 19.3 分割してはいけない組み合わせ

大項目単位で機械的に切ると契約が壊れる箇所があります。次は必ず同一PRに含めてください。

- **JSON Schema の変更と、それを読む loader / codec** — 別PRにすると中間コミットで契約が破綻する
- **`resources/presets/` 配下の全preset** — §22.5 が「1つ修正したら全preset横断監査」を要求しており、scope別に分割すると監査が毎PR繰り返され越境の見落としが増える
- **§16 の Ripple Check で `IMPACTED_IN_BOUNDARY` に挙がった範囲** — 影響先を別PRへ残すと「テストやSchemaだけ古い定義」の状態がmainに乗る

ユニットテストは対象実装と同一PRに含めます（§14）。fixture ベースの横断 integration test と snapshot test のみ、最終PRにまとめて構いません。

### 19.4 PR規模の計測と申告

**「超過していないこと」を記憶で担保しないでください。** 計測は機械が行い、PR作成者は結果を申告します。

計測コマンド:

```bash
python tools/pr_budget.py                  # 人間向け。base は自動判定
python tools/pr_budget.py --format markdown  # PR本文へ貼る申告ブロック
```

PR本文には `## PR Budget` ブロックを必ず含めます（`.github/pull_request_template.md` に空欄あり）。次を満たさないPRはレビュー前に差し戻して構いません。CIも同じ条件で失敗します。

- `## PR Budget` ブロックが存在しない
- 必須フィールド（`BASE` / `REVIEW_LINES` / `REVIEW_FILES` / `TEST_LINES` / `OBSERVATIONS` / `VERDICT`）のいずれかが空
- `OBSERVATIONS` が推定値のまま確認されていない（`(推定` の注記が残っている）
- 申告された `VERDICT` が、実測値と申告分類から導かれる判定と一致しない
- `VERDICT: OVER_BUDGET` なのに `OVER_BUDGET_REASON` が空

`OBSERVATIONS` はカンマ区切りで、各項目を §22.1 の分類記号（`A`〜`E`）で始めます。該当なしは `-` と書きます。

```text
OBSERVATIONS: A 外部入力検証, C ドメイン不変条件
```

判定の分担は次のとおりです。行数とファイル数は機械が確定し、作成者は上書きできません。観点分類は作成者の申告が優先されます（推定は下書きにすぎないため）。

| 判定条件 | 確定者 | 上書き |
|---|---|---|
| `REVIEW_LINES` 超過 | `tools/pr_budget.py` の実測 | 不可 |
| `REVIEW_FILES` 超過 | `tools/pr_budget.py` の実測 | 不可 |
| 観点3分類以上 | 作成者の申告（`OBSERVATIONS`） | 推定値を作成者が訂正可 |

CIは行数・ファイル数の実測と申告分類を突き合わせ、そこから導かれる判定と申告 `VERDICT` の一致を要求します。実測の数値そのものは突き合わせません（コミットのたびに本文更新を強いると、申告が形骸化するため）。

`VERDICT: OVER_BUDGET` は原則として分割します。分割しない場合は `OVER_BUDGET_REASON` に理由を書き、§19.3 の「分割してはいけない組み合わせ」に該当することを示してください。**理由を書けば通します。禁止しているのは、計測せずに出すことです。**

強制経路は三重にします。

1. `kiro-impl` が各タスクのコミット後に計測し、スライス末尾で停止する（§19.5）
2. `.github/workflows/pr-budget.yml` がPR作成・更新時に `tools/pr_budget.py` で計測し、`tools/check_pr_declaration.py` で申告を検証してジョブを失敗させる
3. レビュー時に本節の申告要件を確認する

### 19.5 実装スライスと `_PR:_` 注記

分割判断はPR作成時ではなく、**tasks.md の作成時に1度だけ**行います。`tasks.md` の大項目には、その大項目が属するPRスライスを `_PR:_` で明記してください。
...
- スライス名は `^[a-z0-9-]+$`。ブランチ名は `feat/<feature>_<スライス名>`（§19.1）
- 1スライス = 1PR。`_PR:_` が無い大項目は直前の大項目と同一スライスとみなす
- スライス境界は §19.2 の閾値と §19.3 の「分割してはいけない組み合わせ」の両方を満たすように引く
- `kiro-impl` はスライス単位で実行し、スライス末尾のタスクを完了したら**次のスライスへ進まずに停止**する

これにより、閾値超過は「PR作成時に思い出して確認する作業」ではなく、スライス設計時の1度きりの判断になります。
...
- [ ] 各指摘について根拠確認ゲート（§17.3.1）を実施し、`SPEC_BASIS` を報告に残した
- [ ] 仕様に根拠が無い指摘は、実装で答えず §21.2 の分岐で処理した
- [ ] §22 の頻出指摘チェックリストのうち、該当分類を全て確認した
- [ ] レビューラウンドが10以内で `APPROVED` に到達した
- [ ] `python tools/pr_budget.py` を実行し、PR本文へ `## PR Budget` を申告した（§19.4）
...
### 21.2 仕様に根拠のない指摘を受けた場合

§17.3.1 の根拠確認ゲートで「根拠が実在しない」と判定した指摘は、**実装で答えないでください**。次のいずれかへ分岐します。

**分岐A: 挙動として必要 → 仕様を先に確定させてから実装する**

指摘された挙動が製品として必要だと判断した場合:

1. requirements / design へ**決定表として**追記する。分岐の全入力と全出力を漏れなく定義する（§22.1）
2. 適用範囲の選択肢（例: 文字列全体か、特定位置に限定か）が複数ある場合、**どちらを採るかを仕様で明示的に選び、採らなかった側と受容するトレードオフを書く**
3. 仕様が確定してから実装する。実装は決定表の写像であり、実装側で分岐を発明しない
4. 仕様変更は §2 の優先順位により承認が要る。承認前に実装を先行させない

**分岐B: 挙動として不要、または有害 → 修正せず、根拠を示して返す**

修正しない判断をした場合、返信に次を含めてください。

- 当該指摘が requirements / design のどの条文にも根拠を持たないこと（確認した節番号を挙げる）
- 提案どおり修正すると何が起きるか。**特に、修正が新規欠陥を生む場合は再現入力と実測結果を示す**
- 仕様文言が判定基準を特定していないことが原因である場合は、その文言を明確化した差分

§17.5 のとおり、仕様に根拠の無い指摘は `REJECTED` の理由になりません。ただし「仕様が判定基準を定めていない」という指摘自体は有効なので、分岐Bでも仕様文言の明確化までは行ってください。

### 21.3 未仕様の挙動を実装で発明しない

上記は文字列後処理に限らず**全レイヤーの共通則**です。次のいずれかを実装側の判断で埋めないでください。

- 分岐の出力が仕様で定義されていない入力クラス
- 適用範囲・優先順位・タイブレーク規則
- エラー時の fallback 先とその選択理由
- キャッシュキーの構成要素
- 正規化・修復・後処理の規則

いずれも「正しい答え」が仕様で定義されない限り、レビューのたびに解釈が変わり収束しません（§17.3.1 の実測を参照）。
...
- 修復規則は**承認済み requirements / design に決定表として明記してから実装する**（§21.3 の共通則。文字列後処理はこれを最も踏みやすい分類）
...
- 文字種の判定基準（英数字・空白・単語境界）は**どの集合か**を仕様で特定する。同一関数内で ASCII 固定集合と `\s` / `str.isalnum()` のような Unicode 判定を混在させない。混在は「一方のパスで内容ありと判定されたものが次のパスで消える」種類の欠陥を生む
