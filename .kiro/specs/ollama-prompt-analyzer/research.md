...
- **Trade-offs**: 名前解決が OS 側で停止した場合、放棄したワーカーは resolver が返るまで存続し接続を保持し得る。放棄側は完了時に自身の接続を必ず close し共有状態へ書き込まない規約とする。
- **Follow-up**: 名前解決を意図的に遅延させたスタブで、呼び出しが実効 timeout 以内に必ず戻ることを単体テストで検証する（タスク 3.1）。
- **訂正**: 当初この Trade-offs に「放棄は 1 実行あたり最大 2 件（再試行上限）で有界」と記載したが、**この推論は誤り**である。1 実行あたりの有界性はプロセス全体の有界性を導かない。次の決定で共有プールによる上限へ置き換えた。

### Decision: ワーカーをプロセス全体で共有する上限付きプールから確保する（PR#6 2 巡目レビュー由来）

- **Context**: 上の B-1 で導入したワーカー方式に対し、「DNS 名の resolver が返らない環境で実行を繰り返すと、1 実行あたり最大 2 件でもプロセス全体では蓄積する」との指摘を受けた。前回の修正で新たに混入した欠陥である。
- **根拠確認の結果**: 引用された AGENTS.md §17.5 は「実際に到達可能な入力での性能劣化・セキュリティ問題」を Critical / Important の対象に挙げており、**主張を支持する**（前 2 件の引用が無関係な箇条書きだったのとは異なる）。ただし「ワーカープールを設けよ」という挙動要件は承認済み要件に存在せず、Requirement 12.3（無限再試行と無制限の待機の禁止）も呼び出し側の待機が deadline で有界である以上、違反していない。**修正の根拠は design.md 自身の記述が事実として誤っていた点にある**。
- **指摘のうち正確でない部分**: 「無制限に蓄積」は厳密には過大である。放棄スレッドは resolver が返れば自ら終了するため、定常状態の本数は概ね `resolver 遅延 ÷ 実行間隔` で頭打ちになる。無限に増えるのは resolver が恒久的に応答しない場合に限られる。ただし Requirement 10.7 が `timeout` の下限を 0.1 秒と定め Requirement 10.1 が DNS 名を許容する以上、数十本に達する経路は実在するため、指摘を退ける理由にはならない。
- **最も重要な部分**: 「非 daemon スレッドなら終了も妨げられる」は**蓄積量に関係なく確実に発生する害**である。Python は終了時に非 daemon スレッドを join するため、`getaddrinfo` で停止したスレッドが 1 本でもあれば ComfyUI のプロセス終了がブロックされる。当初設計は daemon 指定に言及していなかった。
- **Alternatives Considered**:
  1. **共有プール + 上限 + daemon 化**（採用）
  2. 名前解決だけをワーカーで行い解決済み IP へ接続する — 放棄を endpoint あたり 1 本に抑えられるが、https で SNI と証明書検証をホスト名に対して行うため専用の接続クラスが必要になり TLS 周りに新たな失敗面を作る。daemon 化の必要性も消えない
  3. `ollama_url` を IP literal に限定する — Requirement 10.1 が DNS 名を明示的に許容しており要件変更が必要
- **Selected Approach**: `MAX_INFLIGHT_TRANSPORT_WORKERS` を上限とする共有プールからワーカーを確保する。スロット獲得待ちは当該試行の deadline 予算の内側で行い、deadline までに空きが出なければ `TransportTimeoutError` とする。ワーカーは daemon とし、放棄側は完了時に接続を close してスロットを解放する。
- **Rationale（§21.3 に抵触しない理由）**: 上限到達時の結果が**既定義の分類 (b) タイムアウト**へ落ちるため、新しい失敗分類も新しい分岐も作らない。`safe_fallback` なら fallback 出力、`strict` なら明示的エラーで、いずれも 24 組合せ表の既存経路である。スロット待ちが deadline 内であるため Requirement 6.6 の上限も維持される。上限値の選択は `CACHE_CAPACITY` と同じ論法で、退避・待機が既定義の経路へ落ちるだけであり観測可能な契約を変えない。
- **Trade-offs**: 全スロットが停止中のワーカーで占有されると以降の実行は分類 (b) へ縮退する。DNS が停止している状況では元より全実行がタイムアウトするため新たな機能低下ではない。
- **Follow-up**: プール容量を超える回数の連続タイムアウト実行で、生存ワーカー数が上限を超えないこと・全ワーカーが daemon であること・上限到達後も各実行が期限以内に戻ることを単体テストで検証する（タスク 3.1）。
